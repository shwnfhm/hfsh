...
print_error_message () {
    ...
    local contrunning=$2
    local filetype=$3
    builtin echo -e "\033[31mtest $testnum: $filetype incorrect\033[0m"
...
run_and_check () {
    ...
    if (( $verbose == 1 )); then
	echo -n -e "\033[33mrunning test $testnum: \033[0m"
	...
	builtin echo -e "\033[32mtest $testnum: passed\033[0m"
	if (( $verbose == 1 )); then
	    echo ""
	fi
    else
...
if (( $skippre == 0 )); then
    if [[ -f tests/pre ]]; then
	builtin echo -e "\033[33mdoing one-time pre-test\033[0m (use -s to suppress)"
	source tests/pre
	if (( $? != 0 )); then
